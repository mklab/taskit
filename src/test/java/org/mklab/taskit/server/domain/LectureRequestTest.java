...
public class LectureRequestTest extends DomainTest {
  ...
  public void testGetAllLectures() {
    ...
    register(req, "title1", "description1", new Date()); //$NON-NLS-1$//$NON-NLS-2$
    register(req, "title2", "description2", new Date(System.currentTimeMillis() + TimeUnit.DAYS.toMillis(7))); //$NON-NLS-1$//$NON-NLS-2$
    register(req, "title3", "description3", new Date(System.currentTimeMillis() + TimeUnit.DAYS.toMillis(14))); //$NON-NLS-1$//$NON-NLS-2$
    req.fire();
    ...
    req.getAllLectures().fire(new Receiver<List<LectureProxy>>() {
      ...
      public void onSuccess(List<LectureProxy> response) {
        ...
        System.out.println(response.get(0).getReports());
  ...
  private static void register(LectureRequest req, String title, String description, Date date) {
